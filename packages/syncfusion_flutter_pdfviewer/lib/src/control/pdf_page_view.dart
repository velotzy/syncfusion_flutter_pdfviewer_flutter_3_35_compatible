...
class PdfPageViewState extends State<PdfPageView> {
  ...
  Widget build(BuildContext context) {
    final double pageSpacing = widget.pageIndex == widget.pdfViewerController.pageCount - 1
        ? 0.0
        : widget.pageSpacing;
                  ...
                  Stack(
                    children: <Widget>[
                      SizedBox(
                        ...
                        child: Semantics(label: widget.semanticLabel, child: _pdfPage),
                      ),
                ...
                child: KeyboardListener(
                  ...
                  onKeyEvent: (KeyEvent event) {
                    ...
                    if (selectionDetails != null &&
                        (selectionDetails.mouseSelectionEnabled || selectionDetails.selectionEnabled) &&
                        isPrimaryKeyPressed &&
                    ...
                    if (event is KeyDownEvent) {
                      if (event.logicalKey == LogicalKeyboardKey.home ||
                        ...
                        if (widget.selectedAnnotation != null) {
                          widget.pdfViewerController.removeAnnotation(widget.selectedAnnotation!);
                        }
                      }
                    }
                      ...
                      setState(() {
                        if (canvasRenderBox != null && widget.pdfPages.isNotEmpty) {
                          ...
                          if (_interactionMode == PdfInteractionMode.selection) {
                            ...
                            final bool isTOC = canvasRenderBox?.findTOC(details.localPosition) ?? false;
                            if (isTOC) {
  ...
  void _calculateHeightPercentage() {
    ...
    _heightPercentage =
        (_isRotatedTo90or270 ? _originalPageSize.width : _originalPageSize.height) /
  ...
  Future<void> _getImage(Size viewportSize, double zoomLevel) async {
    ...
    if (widget.pdfDocument != null && _previousImageFactor != imageFactor) {
      ...
      if (ratio < 0.5 ||
        ...
        if (originalPageSize.width * originalPageSize.height * 4 <= 5 * 1024 * 1024) {
      ...
      try {
        ...
        await _pageImageOperation?.value.then((Uint8List? pageImage) {
          if (pageImage != null) {
            _createImage(pageImage, _imageWidth, _imageHeight).then((ui.Image image) {
              if (mounted) {
                setState(() {
  ...
  void _addTextMarkupAnnotation(String type) {
    ...
    if (selectedLines != null && selectedLines.isNotEmpty) {
      ...
      if (type == 'highlight') {
        ...
        annotation = StrikethroughAnnotation(textBoundsCollection: selectedLines);
  ...
  Future<void> getTileImage(
    ...
    if (!exposed.isEmpty && !exposed.hasNaN) {
      ...
      if (zoomLevel == transformationController.value[0]) {
        try {
          final Size tileImageSize = Size(exposed.width * _dpr * ratio, exposed.height * _dpr * ratio);
          ...
          await imageFuture.then((Uint8List? tileImage) {
            if (tileImage != null && zoomLevel == transformationController.value[0]) {
              _createImage(tileImage, tileImageSize.width.toInt(), tileImageSize.height.toInt()).then((
                ui.Image image,
              ) {
                if (mounted) {
                  setState(() {
